    ...
    for (auto it = e.begin(); it != e.end(); ) { 
        ...
        if (ent->getId() == 2 && CheckCollisionRecs(hitbox, ent->getHitbox())) {
            ...
            if (CheckCollisionPointRec(top, ent->getHitbox())) { //collision above (head)
                ...
                if (BreakBlock* breakb = dynamic_cast<BreakBlock*>(ent)) { //do a dynamyc cast to know if that pointer has a surpirseBlock element to acces to it

                    breakb->markForDelation();
